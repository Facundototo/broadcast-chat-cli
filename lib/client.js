...
import { CLIENT_CLOSED, SERVER_SHUTTING_DOWN, CLIENT_PROMPT, SERVER_PROMPT, SERVER_SUCCESSFUL_NAME, HELP_ADVICE, SEND_COMMAND, SEND_TO_COMMAND, CLOSE_COMMAND, SET_NAME_COMMAND, HELP_COMMAND, CLEAR_COMMAND, JOIN_CHANNEL_COMMAND,printChatChannel, SERVER_CHANNEL_JOINED } from '../utils/utils.js';
...
function manageInput(args,ws) {
   ...
   switch(args[0]){
      ...
      case SET_NAME_COMMAND: ws.send(JSON.stringify({option:SET_NAME_COMMAND,arg:args[1]}));
         break;
      // /join <channel>
      case JOIN_CHANNEL_COMMAND: ws.send(JSON.stringify({option:JOIN_CHANNEL_COMMAND,arg:args[1]}));
         break;
      // /clear

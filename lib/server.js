...
import { SERVER_CLIENT_CONNECTED, SERVER_CLIENT_DISCONNECTED, SERVER_CLIENT_NAME, SERVER_NAME_ALERT, SERVER_PROMPT, SERVER_SHUTTING_DOWN } from '../utils/messages.js';
...
function manageMsg(message,wss,ws){
    ...
    if(option === 'send-to-all'){
        ...
        ws.username = arg; 
        ws.prompt =`<USER ${arg}> `;
        ws.send(JSON.stringify({prompt:SERVER_PROMPT,msg:SERVER_CLIENT_NAME,new_username:arg}));
    }
}
...
import { SEND_COMMAND, SEND_TO_COMMAND, SERVER_CLIENT_CONNECTED, SERVER_CLIENT_DISCONNECTED, SERVER_NAME_ALERT, SERVER_PRIVATE_MESSAGE_ERROR, SERVER_PROMPT, SERVER_SHUTTING_DOWN, SET_NAME_COMMAND } from '../utils/utils.js';
import { setName,sendToAll, sendPrivateMessage } from './commands/commands.js';
...
function manageMsg(message,wss,ws){
    ...
    if((option === SEND_COMMAND || option === SEND_TO_COMMAND) && ws.username === undefined){
        ws.send(JSON.stringify({prompt:SERVER_PROMPT,msg:SERVER_NAME_ALERT}));
    }
        
    switch(option){
        case SEND_COMMAND: sendToAll(arg,ws.prompt,wss);
            break;

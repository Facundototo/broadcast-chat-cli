...
import { SERVER_CLIENT_CONNECTED, SERVER_CLIENT_DISCONNECTED, SERVER_NAME_ALERT, SERVER_PROMPT, SERVER_SHUTTING_DOWN } from '../utils/utils.js';
import { setName,sendToAll } from './commands/commands.js';
...
function manageMsg(message,wss,ws){
    ...
    switch(option){
        ...
        case 'send-to-user':
            const { user,msg } = arg;
            const user_exists = validateUsernames(user,wss);
            // ***
            break;
        case 'set-name':
            setName(arg,wss,ws);

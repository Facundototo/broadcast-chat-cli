...
import { SERVER_CLIENT_CONNECTED, SERVER_CLIENT_DISCONNECTED, SERVER_CLIENT_NAME, SERVER_NAME_ALERT, SERVER_SHUTTING_DOWN } from '../utils/messages.js';
...
export const startServer = () => {
    ...
    wss.on('connection',(ws) => {
        ...
        setTimeout(() =>{
            ...
            sendToAll(`${SERVER_CLIENT_CONNECTED} ${(ws.username !== undefined)?`-> *${ws.username}*`:``}`,wss);
        },500)
        ...
        ws.on('close', () => {
                ...
                (ws.username !== undefined) && sendToAll(`${SERVER_CLIENT_DISCONNECTED}: ${ws.username}`,wss)
            }
        );
...
function closeServer(wss) {
    ...
    sendToAll(SERVER_SHUTTING_DOWN,wss);
...
function manageMsg(message,wss,ws){
    ...
    if(option === 'send-to-all'){
        if(ws.username === undefined) ws.send(SERVER_NAME_ALERT);
        else sendToAll(`${ws.username} send: ${arg}`,wss);
    } 
    else if(option === 'set-name') {
        // set the name in ws.username
        ws.username = arg;
        ws.send(SERVER_CLIENT_NAME);
    }
}
...
import { JOIN_CHANNEL_COMMAND, SEND_COMMAND, SEND_TO_COMMAND, SERVER_CLIENT_CONNECTED, SERVER_CLIENT_DISCONNECTED, SERVER_NAME_ALERT, SERVER_PROMPT, SERVER_SHUTTING_DOWN, SET_NAME_COMMAND } from '../../utils/messages.js';
...
function manageMsg(message,wss,ws){
    ...
    if((option === SEND_COMMAND || option === SEND_TO_COMMAND || option === JOIN_CHANNEL_COMMAND) && ws.username === undefined){
        sendTo(ws,{prompt:SERVER_PROMPT,msg:SERVER_NAME_ALERT});
    }else{  
        switch(option){

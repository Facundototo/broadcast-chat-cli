...
import { JOIN_CHANNEL_COMMAND, LIST_USERS_COMMAND, SEND_COMMAND, SEND_TO_COMMAND, SERVER_CLIENT_CONNECTED, SERVER_CLIENT_DISCONNECTED, SERVER_NAME_ALERT, SERVER_PROMPT, SERVER_SHUTTING_DOWN, SET_NAME_COMMAND } from '../../utils/messages.js';
...
function manageMsg(message,wss,ws){
    ...
    if((option === SEND_COMMAND || option === SEND_TO_COMMAND || option === JOIN_CHANNEL_COMMAND) && ws.username === undefined){
        sendTo(ws,{prompt:SERVER_PROMPT,msg:SERVER_NAME_ALERT,show_msg:true});
    }else{  
        switch(option){
            ...
            case SET_NAME_COMMAND: setName(arg,wss,ws);
                break;
            case JOIN_CHANNEL_COMMAND: joinChannel(arg,ws);
                break;   
            case LIST_USERS_COMMAND: returnClientsByChannel(wss,ws,arg);
                break;
        }
    }
}

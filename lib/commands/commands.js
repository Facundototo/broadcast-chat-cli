...
export function sendToAll(msg,prompt,channel,wss){
    ...
    wss.clients.forEach(client => {
        ...
        const channel_to_send = (channel) ? (client.channel === channel) : true;
...
export function printHelp(){
 ...
 export function joinChannel(arg,ws,channels){
    ...
    if(channel_finded){
        ...
        ws.send(JSON.stringify({prompt:SERVER_PROMPT,msg:SERVER_CHANNEL_JOINED,channel:channel_finded}));
        //buffer
    }else{
        ws.send(JSON.stringify({prompt:SERVER_PROMPT,msg:SERVER_CHANNEL_NOT_FINDED}));
    }
 }
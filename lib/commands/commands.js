import { SERVER_PROMPT, SERVER_NO_SUCCESSFUL_NAME, SERVER_SUCCESSFUL_NAME, SERVER_PRIVATE_MESSAGE_ERROR, SERVER_CHANNEL_NOT_FINDED, SERVER_CHANNEL_JOINED } from '../../utils/messages.js';
...
export function setName(arg,wss,ws){
    ...
    if (arg.trim().length === 0 || user_exists){
        sendTo(ws,{prompt:SERVER_PROMPT,msg:SERVER_NO_SUCCESSFUL_NAME});
    }else if(!user_exists){
        ...
        ws.username = arg; 
        ws.prompt =`<USER ${arg}> `;
        sendTo(ws,{prompt:SERVER_PROMPT,msg:SERVER_SUCCESSFUL_NAME,new_username:arg});
    }
...
export function sendPrivateMessage(arg,ws,wss){
    ...
    if(!user_find || user_find.username === ws.username){
        sendTo(ws,{prompt:SERVER_PROMPT,msg:SERVER_PRIVATE_MESSAGE_ERROR});
    } else {
        sendTo(user_find,{prompt:`<FROM ${ws.username}> `,msg:msg});
        sendTo(ws,{prompt:`<TO ${user_find.username}> `,msg:msg});         
    }
}
...
export function printHelp(){
 ...
 export function joinChannel(arg,ws,channels){
    const channel_finded = channels.find(channel => channel === arg);
    if(channel_finded){
        ws.channel = channel_finded; 
        sendTo(ws,{prompt:SERVER_PROMPT,msg:SERVER_CHANNEL_JOINED,channel:channel_finded});
        //buffer
    }else{
        sendTo(ws,{prompt:SERVER_PROMPT,msg:SERVER_CHANNEL_NOT_FINDED});
    }
 }
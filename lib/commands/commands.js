import { SERVER_PROMPT, SERVER_NO_SUCCESSFUL_NAME, SERVER_SUCCESSFUL_NAME, SERVER_PRIVATE_MESSAGE_ERROR, SERVER_CHANNEL_NOT_FINDED, SERVER_CHANNEL_JOINED, SERVER_USERS_FILTERED_BYCHANNEL } from '../../utils/messages.js';
import { sendTo } from '../../utils/utils.js';
import { channels } from '../server/data.js';
...
export function setName(arg,wss,ws){
    ...
    if (arg.trim().length === 0 || user_exists){
        sendTo(ws,{prompt:SERVER_PROMPT,msg:SERVER_NO_SUCCESSFUL_NAME, show_msg:true});
    }else if(!user_exists){
        //sent to all that a client has changed his name
        if(ws.username) sendToAll(`user "${ws.username}" changed his name to "${arg}"`,SERVER_PROMPT,ws.channel,wss)
        // set the name in ws.username  
        ws.username = arg; 
        ws.prompt =`<USER ${arg}> `;
        sendTo(ws,{prompt:SERVER_PROMPT,msg:SERVER_SUCCESSFUL_NAME,show_msg:true,new_username:arg});
    }
...
export function sendPrivateMessage(arg,ws,wss){
    ...
    if(!user_find || user_find.username === ws.username){
        sendTo(ws,{prompt:SERVER_PROMPT,msg:SERVER_PRIVATE_MESSAGE_ERROR,show_msg:true});
    } else {
...
export function printHelp(){
 ...
 export function joinChannel(arg,ws){
    ...
    if(channel_finded){
        ...
        sendTo(ws,{prompt:SERVER_PROMPT,msg:SERVER_CHANNEL_JOINED,show_msg:true,channel:channel_finded});
        //buffer
    }else{
        sendTo(ws,{prompt:SERVER_PROMPT,msg:SERVER_CHANNEL_NOT_FINDED,show_msg:true});
    }
 }
...
export function returnClientsByChannel(wss,ws,channel_option){
    ...
    sendTo(ws,{prompt:SERVER_PROMPT,msg:SERVER_USERS_FILTERED_BYCHANNEL, show_msg:false, users: [...filtered_clients]});
}

...
export const connectToServer = (username,port) => {
   ...
   ws.on('message', (data) => {
      ...
      const { prompt:newPrompt,msg,showMsg } = parsedData;
      
      // if the new_prompt is equal to my prompt, print <ME>
      prompt((newPrompt === ws.prompt)?'<ME> ':newPrompt);
      (showMsg) && console.log(msg.toString());

      if(newPrompt === Server.PROMPT){
         switch(msg){
            // if the new name changed successfully, set the new elements 
            case Server.SUCCESSFUL_NAME: setWS(`<USER ${parsedData.new_username}> `,parsedData.new_username,ws);
               break;
            // join a channel
            ...
            case Server.USERS_FILTERED_BYCHANNEL: listUsers(ws,parsedData.users);
               break;
            // server closes
